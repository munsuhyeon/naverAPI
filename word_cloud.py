...
inputFileName = 'bts_네이버_news.json'
...
word_count = dict() # 추출한 단어들을 저장한 빈 dict 선언
for tag,counts in count.most_common(80):
    if len(tag) > 1: # tag(추출한 단어)의 길이가 1보다 큰 단어만 추출
        word_count[tag] = counts

font_path = 'C:\Windows\Fonts\margun.ttf'
font_name = font_manager.FontProperties(fname=font_path).get_name() #그래프에 한글표시
